from data import *
from utils.augmentations import SSDAugmentation, augment_bbox, augment_bboxes
...
def train():
    ...
    while(finish_flag):
        ...
        for iteration in range(args.start_iter, cfg['max_iter']):
            ...
            #         inv_loc = augment_bbox(bbox,inv_mat)
            #         batched_inv_locs.append(inv_loc)
            ...
            #########################
            # supervised losses END
            
            # consistency losses START
            ##########################
            ...
            if(mask_val.sum()>0):
                ## JSD !!!!!1
                conf_sampled_aug = conf_sampled_aug + 1e-7
                ...
                # PERTURB LOSS
                perturb_loss = torch.sum(1 - cos_sim(features,features_aug))
            ...
            if iteration % 10 == 0:
                ...
                print('loss2: %.4f , loss_c: %.4f , loss_l: %.4f , loss_con: %.4f, loss_con_loc: %.4f, loss_con_conf: %.4f, loss_pert: %.4f, lr : %.4f, super_len : %d\n' % (loss2.data, loss_c.data, loss_l.data, consistency_loss.data, consistency_conf_loss.data, consistency_loc_loss.data, perturb_loss.data,float(optimizer1.param_groups[0]['lr']),len(sup_image_index)))
