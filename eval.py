...
def do_python_eval(output_file='test.txt',output_dir='output', use_07=True):
    with open(output_file,'w') as file:
        ...
        for i, cls in enumerate(labelmap):
            ...
            aps += [ap]
            file.write('AP for {} = {:.4f}\n'.format(cls, ap))
            # with open(os.path.join(output_dir, cls + '_pr.pkl'), 'wb') as f:
            #     pickle.dump({'rec': rec, 'prec': prec, 'ap': ap}, f)
        file.write('Mean AP = {:.4f}\n'.format(np.mean(aps)))
        file.write('~~~~~~~~\n')
        file.write('Results:\n')
        for ap in aps:
            file.write('{:.3f}\n'.format(ap))
        ...
        file.write('\n')
        file.write('--------------------------------------------------------------\n')
        file.write('Results computed with the **unofficial** Python eval code.\n')
        file.write('Results should be very close to the official MATLAB eval code.\n')
        file.write('--------------------------------------------------------------\n')
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
             im_size=300, thresh=0.05, output_filename='test.txt'):
    ...
    evaluate_detections(all_boxes, os.path.join(save_folder,output_filename), output_dir, dataset)


def evaluate_detections(box_list, output_file, output_dir, dataset):
    write_voc_results_file(box_list, dataset)
    do_python_eval(output_file=output_file, output_dir=output_dir)


if __name__ == '__main__':
    # load net
    if args.all_weights is not None:
        assert os.path.exists(args.all_weights)
        list_of_weights = [os.path.join(args.all_weights,x) for x in os.listdir(args.all_weights)]
    else: 
